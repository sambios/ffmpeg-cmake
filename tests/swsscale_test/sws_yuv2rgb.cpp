...
int main(int argc, char* argv[])
{
    ...
    while(1){
        ...
        if(packet->stream_index==videoindex){
            //decode_frame(pCodecCtx, pFrame, packet);
            ...
            std::cout << "got_picture = " << got_picture << ", cache " << cache_count;

            if(got_picture){
                frame_idx++;

                SwsContext *sws_ctx = sws_getContext(pFrame->width, pFrame->height, (AVPixelFormat)pFrame->format, 1280, 720, AV_PIX_FMT_RGB32, SWS_POINT, 0, 0, 0);
                AVFrame *scaledFrame = av_frame_alloc();
                scaledFrame->width = 1280;
                scaledFrame->height = 720;
                scaledFrame->format = AV_PIX_FMT_RGB32;
                ret = av_frame_get_buffer(scaledFrame, 8);
                assert(ret == 0);
                ret = sws_scale(sws_ctx, pFrame->data, pFrame->linesize, 0, pFrame->height, scaledFrame->data, scaledFrame->linesize);
                assert(ret >= 0);
